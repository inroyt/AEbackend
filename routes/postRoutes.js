...
router.post('/addPost',upload.single('image'),parseDataMiddleware, [
  ...
  check('data.title').isLength({ max: 1000 }).withMessage('Title should not exceed 100 characters'),
  check('data.details').notEmpty().withMessage('Content is required'),
 // check('data.details').isLength({ max: 15000 }).withMessage('Content should not exceed 5000 characters'),
